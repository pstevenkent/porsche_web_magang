...
export default function AddProductForm({ initialData, onSave }) {
  ...
  const handleSubmit = async (e) => {
    ...
    formData.powertrainperformance
      .filter((p) => p.trim() !== "")
      .forEach((p) => {
    ...
    if (imageFiles.length > 0) {
      ...
      // Setelah semua upload selesai, masukkan CID-nya ke FormData
      uploads.forEach((cid) => {
        console.log(cid);
        data.append("images", cid);
      });
    }
    ...
    try {
      ...
      if (!response.ok || result.status !== "success") {
        throw new Error(
          result.message ||
            `Gagal ${isEditMode ? "memperbarui" : "menambahkan"} produk.`
        );
      }
      setMessage(
        `Sukses! Produk berhasil ${isEditMode ? "diperbarui" : "ditambahkan"}.`
      );
      if (!isEditMode) {
      ...
      <form onSubmit={handleSubmit} className="space-y-8">
        ...
        <div className="grid grid-cols-1 gap-6 md:grid-cols-2">
          <div>
            <label
              htmlFor="vehicle"
              className="mb-2 block text-sm font-bold text-porscheGray-dark"
            >
              Vehicle Name
            </label>
            <input
              ...
              className="w-full rounded-lg border border-porscheGray p-3"
              required
            />
          </div>
          <div>
        ...
        <div className="grid grid-cols-1 gap-6 md:grid-cols-2">
          ...
          <div>
            ...
            <input
              type="text"
              id="interiorcolours"
              name="interiorcolours"
              value={formData.interiorcolours}
              onChange={handleChange}
              className="w-full rounded-lg border border-porscheGray p-3"
            />
          </div>
        ...
        <div>
          ...
          <input
            ...
            id="rooftransport"
            name="rooftransport"
            value={formData.rooftransport}
            onChange={handleChange}
            className="w-full rounded-lg border border-porscheGray p-3"
          />
        </div>
        <div>
          <label className="mb-2 block text-sm font-bold text-porscheGray-dark">
          ...
          <div className="space-y-3">
            {formData.powertrainperformance.map((perf, index) => (
              <DynamicInputField
                ...
                value={perf}
                onChange={(e) =>
                  handleDynamicChange(e, index, "powertrainperformance")
                }
                onRemove={() =>
                  handleRemoveItem(index, "powertrainperformance")
                }
              />
            ))}
          </div>
          <button
            type="button"
            onClick={() => handleAddItem("powertrainperformance")}
            className="mt-3 rounded-lg border border-porscheGray bg-porscheGray-light px-4 py-2 text-sm font-bold text-porscheBlack transition hover:bg-porscheGray"
          >
            + Add Feature
          </button>
        </div>
